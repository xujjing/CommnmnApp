...
import tv.danmaku.ijk.media.widget.media.AbstractMediaPlayer;
import tv.danmaku.ijk.media.widget.media.IMediaPlayer;
import tv.danmaku.ijk.media.widget.media.IjkLibLoader;
import tv.danmaku.ijk.media.widget.media.IjkMediaCodecInfo;
import tv.danmaku.ijk.media.widget.media.IjkMediaMeta;
import tv.danmaku.ijk.media.widget.media.IjkTimedText;
import tv.danmaku.ijk.media.widget.media.MediaInfo;
...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    public static final int FFP_OPT_CATEGORY_PLAYER = 4;

    public static final int PLAY_TYPE_LIVE = 111;
    public static final int PLAY_TYPE_LIVE_PLAYBACK  = 112;
    public static final int PLAY_TYPE_LIVE_TIMESHIFT  = 112;
    public static final int PLAY_TYPE_VOD   = 113;
    ...
    private static final int MEDIA_REC_STATE = 2050; ///录制
    private static final int MEDIA_REC_PROGRESS = 2051; ///录制进度
    private static final int MEDIA_LINE_DETECTION = 2055;///线路探测
    private static final int REC_MEG_READ_PACKET = 9;//播放器播放时  读取到新的媒体数据
    ...
    // TODO: @Override
    public int selectTrack(int track) {
        return _setStreamSelected(track, true);
    }
    ...
    private native int _setStreamSelected(int stream, boolean select);
    ...
    @Override
    public void setOnRecordingProgressListener(OnRecordingProgressListener listener) {
        mOnRecordingProgressListener = listener;
    }

    @Override
    public void setOnPullStreamListener(OnPullStreamListener listener) {
        mOnPullStreamListener = listener;
    }

    @Override
    public void setOnLineDetectionListener(OnLineDetectionListener lineDetectionListener) {
        mOnLineDetectionListener = lineDetectionListener;
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case MEDIA_PLAYBACK_COMPLETE:
                    DebugLog.i(TAG, "handleMessage================MEDIA_PLAYBACK_COMPLETE");
                ...
                case MEDIA_ERROR:
                    DebugLog.i(TAG, "handleMessage================MEDIA_PLAYBACK_COMPLETE");
                    DebugLog.e(TAG, "MEDIA_ERROR_Error (" + msg.arg1 + "," + msg.arg2 + ")");
                ...
                case MEDIA_SET_VIDEO_SAR:
                    ...
                    player.notifyOnVideoSizeChanged(player.mVideoWidth, player.mVideoHeight,
                            player.mVideoSarNum, player.mVideoSarDen);
                    break;
                case MEDIA_REC_STATE:
                    DebugLog.i(TAG, "rec state (" + msg.arg1 + "," + msg.arg2 + ")");
                    player.notifyRecordingStatus(msg.arg1, msg.arg2);
                    if (msg.arg2 == REC_MEG_READ_PACKET) {
                        player.notifyPullStream();
                    }
                    break;
                case MEDIA_REC_PROGRESS:
                    DebugLog.i(TAG, "rec progress count==>" + msg.arg2);
                    player.notifyRecordingProgress(msg.arg2);
                    break;
                case MEDIA_LINE_DETECTION:
                    DebugLog.i(TAG, "rec progress count==>" + msg.arg2);
                    if (msg.obj == null) {
                        player.notifyLineDetection(msg.arg2, "");
                    } else {
                        player.notifyLineDetection(msg.arg2, (String) msg.obj);
                    }
                    break;
                default:
    ...
    public interface OnNativeInvokeListener {
        ...
        int AVAPP_EVENT_TS_RATE = 5;
        int AVAPP_EVENT_TS_RES_TIME = 6;
        String ARG_URL = "url";
    ...
    private static long firstBackTime = 0;

    @CalledByNative
    private static boolean onNativeInvoke(Object weakThiz, int what, Bundle args) {
       /// DebugLog.i(TAG, "onNativeInvoke_what=>" + what + "###间隔时间=>" + (currentTimeMillis() - firstBackTime));
        ...
        if (listener != null &&
                (what == OnNativeInvokeListener.AVAPP_EVENT_TS_RATE ||
                        what == OnNativeInvokeListener.AVAPP_EVENT_TS_RES_TIME) &&
                currentTimeMillis() - firstBackTime >= 5000) {
            firstBackTime = currentTimeMillis();
            listener.onNativeInvoke(what, args);
            return true;
        }
